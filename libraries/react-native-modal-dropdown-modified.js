...
export default class ModalDropdown extends Component {
  ...
  select(idx) {
    ...
    if (idx >= 0) {
      if (this.props.renderHeadText) { value = this.props.renderHeadText(this.props.options[idx]) } else { value = this.props.options[idx].toString() }
    }
  ...
  _onRowPress(rowData, sectionID, rowID, highlightRow) {
    if (!this.props.onSelect ||
      ...
      let buttonText = ''
      console.log(this)
      if (this.props.renderHeadText) { buttonText = this.props.renderHeadText(rowData) } else { buttonText = rowData.toString() }
      this.setState({
        buttonText,
        selectedIndex: rowID,
      })
